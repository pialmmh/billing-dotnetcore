...
- `@b413111` — **Sf10/Sf11 service families** (`src/Billing.Mediation/ServiceFamilies/`): `SfA2ZWithVatTax` (SF10: A2Z amount + VAT=amount×OtherAmount3), `SfDomOffNetInAns` (SF11: ANS=billed×(rateamount−IOF)/60, IOF=OtherAmount1, + BTRC=ANS×OtherAmount3). Each builds the `acc_chargeable` (charge+tax+identity) via `ChargeableBuilder` + mutates the cdr. `BasicCharge` now returns the `acc_chargeable` via the SG's family (SG10→SF10, SG11→SF11); `TierSettlement` gained ServiceFamilyId+Tax. **Deferred:** GL posting/BillingRule/acc_transaction/AutoIncrement/job (mem-ledger + scaffolding), SF11 service-tuple resolution, legacy `RoundFractionsUpTo` (uses decimal.Round). +2 tests.
- `@7196c8d` — **CdrSummaryBuilder** (`src/Billing.Mediation/Summary/`): builds the per-call `sum_voice_*` from cdr + customer chargeable — port of `CdrSummaryFactory.CreateInstanceWithoutDate` (identity/counts/durations) + day/hour bucketing + each SG's `SetServiceGroupWiseSummaryParams` (SG10→day_03/hr_03, SG11→day_02/hr_02). `GetTupleKey()`/`Merge()` drive the rollup. +4 tests.
- `@6917e5c` — **SummaryCache**: accumulate-then-merge per-call summaries by `GetTupleKey()` (one cache per tier schema; the deferred write persists `Items`). +2 tests.
- `@f53598a` — **Extended legs: SG10 supplier leg** (`ServiceFamilies/SfA2Z.cs`, SF1). `BasicCharge` selects the family by `(serviceGroup, direction)` (SG10/Cust→SF10, SG10/Supp→SF1, SG11/Cust→SF11). `FinalizeEngine` `TierMode.Full` runs customer+supplier on the same cdr (supplier taxes on InPartnerCost → must follow customer); `TierSettlement`+SupplierCost, `FinalizeFacts`+OutPartnerId; `CdrSummaryBuilder` SG10 fills suppliercost/supplierrate/tax2 from the cdr. +4 tests.
- **Build 0/0, 56 tests green.** Full chain incl. admin supplier leg: detect → resolve → PrefixMatcher → A2ZRater → family (SF10 cust / SF1 supp / SF11 cust) → acc_chargeable → sum_voice_*. Remaining deferred: ANS extended leg (vat=ZAmount / anscost=CostAnsIn), the single-connection cdr/summary **WRITE** (needs live DB), and the **gRPC** proto reshape + FinalizeAndSummarize adapter (needs architect).
