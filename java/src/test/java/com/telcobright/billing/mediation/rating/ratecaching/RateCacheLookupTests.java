 ...
 * and PrefixMatcher longest-prefixes over that day's per-tuple prefix dictionaries (of {@link Rateext}).
 *
 * <p>The C# {@code Func<>}-backed in-memory loader becomes a {@link Function}-backed one; the dict built with a
 * {@code TupleByPeriod.EqualityComparer} becomes a plain {@link HashMap} (Java uses the key's own
 * equals()/hashCode(), the same value-equality).</p>
 */
class RateCacheLookupTests {

    private static final class InMemoryRateLoader implements IRateLoader {
        private final Function<DateRange, Map<TupleByPeriod, Map<String, List<Rateext>>>> _load;
        int Loads = 0;

        InMemoryRateLoader(Function<DateRange, Map<TupleByPeriod, Map<String, List<Rateext>>>> load) {
            _load = load;
        }
    ...
    private static Map<TupleByPeriod, Map<String, List<Rateext>>> OneTupleDay(DateRange d) {
        Map<String, List<Rateext>> prefixDic = new HashMap<>();
        prefixDic.put("1", new ArrayList<>(List.of(TestData.Ra(1, "1.0").rex())));
        prefixDic.put("17", new ArrayList<>(List.of(TestData.Ra(17, "2.0").rex())));
        prefixDic.put("171", new ArrayList<>(List.of(TestData.Ra(171, "3.0").rex())));

        Map<TupleByPeriod, Map<String, List<Rateext>>> result = new HashMap<>();
