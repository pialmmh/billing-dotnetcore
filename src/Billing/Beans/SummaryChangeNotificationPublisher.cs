...
/// <summary>
/// Best-effort Kafka change-notification for the decoupled summary path. After a cdr batch commits in OUTBOX mode, this
...
/// <c>summary_affected</c> row. This is the only Kafka <i>producer</i> in the host.
///
/// <para><see cref="Publish"/> NEVER throws and NEVER blocks the request: the outbox row is already committed and
...
/// </summary>
public sealed class SummaryChangeNotificationPublisher : IDisposable
{
    private readonly IProducer<Null, string>? _producer;
    private readonly string _topic;
    private readonly ILogger<SummaryChangeNotificationPublisher> _log;

    public SummaryChangeNotificationPublisher(IOptions<SummaryOutboxOptions> opts, ILogger<SummaryChangeNotificationPublisher> log)
    {
        _log = log;
        var o = opts.Value;
        _topic = o.PingTopic;
        if (o.Enabled && !string.IsNullOrWhiteSpace(o.BootstrapServers))
        {
            try
            {
                ...
                _log.LogInformation("summary change-notification publisher → topic {Topic} (servers={Servers})", _topic, o.BootstrapServers);
            }
            catch (Exception ex)
            {
                _log.LogWarning(ex, "summary change-notification publisher init failed; notifications disabled (summary-service will poll)");
            }
        }
    }
    ...
    public void Publish(string tenant, string entityType, int rows)
    {
        ...
        {
            _log.LogWarning(ex, "summary change-notification failed for tenant {Tenant} (non-fatal; summary-service polls)", tenant);
        }
    }
